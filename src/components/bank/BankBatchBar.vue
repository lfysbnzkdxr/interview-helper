...
<template>
  <div v-if="selectedCount > 0" class="flex items-center gap-3 mb-3 p-2 bg-yellow-50 rounded-lg border border-yellow-200 flex-wrap">
    <span class="text-sm text-yellow-700">已选 {{ selectedCount }} 题</span>
    <select :value="batchCategory" @change="onCategoryChange($event.target.value)" class="px-2 py-1 rounded text-xs border border-gray-300 bg-white">
    ...
    <div v-if="showBatchNewCategory" class="flex items-center gap-1">
      ...
      <button @click="emit('cancel-new-category')" class="px-2 py-1 rounded text-xs font-medium bg-gray-200 text-gray-600 hover:bg-gray-300">取消</button>
    </div>
